...
if (!WIZARR || !WIZARR_API_KEY || !STRIPE_WEBHOOK_SECRET) {
  console.error('Missing WIZARR_BASE_URL / WIZARR_API_KEY / STRIPE_WEBHOOK_SECRET (use --env-file=.env)');
  process.exit(2);
}
...
const wz = (path, opts = {}) =>
  fetch(`${WIZARR}${path}`, {
    ...opts,
    headers: { 'X-API-Key': WIZARR_API_KEY, 'Content-Type': 'application/json', ...(opts.headers || {}) },
  });
...
function signature(payload) {
  ...
  const sig = crypto.createHmac('sha256', STRIPE_WEBHOOK_SECRET).update(`${ts}.${payload}`).digest('hex');
  return `t=${ts},v1=${sig}`;
}
...
const fmt = (u) => `  id=${String(u.id).padEnd(4)} ${String(u.server).padEnd(10)} expires=${u.expires}`;
...
async function main() {
  ...
  console.log(await postEvent({
    id: `evt_e2e_checkout_${Date.now()}`,
    object: 'event',
    type: 'checkout.session.completed',
    data: { object: { id: 'cs_e2e', object: 'checkout.session', customer: CUSTOMER, customer_details: { email: EMAIL } } },
  }));
