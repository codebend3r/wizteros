...
PlayKind = Literal["movie", "episode", "track"]
PlayQuality = Literal["4k", "1080p", "720p", "other"]
TopMetric = Literal["plays", "rewatches"]
...
class PlaysOverviewView:
    window: PlaysWindow
    totals: plays.Totals
    by_kind: list[plays.KindCount]
    by_quality: list[plays.QualityCount]
    by_host: list[plays.HostCount]
    timeline: plays.Timeline
    top_viewers: list[plays.Viewer]
    top_titles: list[plays.TopTitle]


@dataclass(frozen=True, slots=True)
class PlayUsersView:
    users: list[plays.User]
...
class TopTitlesView:
    ...
    metric: str
    titles: list[plays.TopTitle]


@dataclass(frozen=True, slots=True)
class PlaySyncView:
    lookback_days: int
    servers: list[plays.ServerStatus]


def _plex_host_names() -> tuple[str, ...]:
    ...
    return tuple(host.name for host in config.HOSTS if host.plex_url)


def play_query(
    days: int = Query(default=DEFAULT_PLAY_DAYS, ge=0, le=MAX_PLAY_DAYS),
    host: str | None = Query(default=None),
    kind: PlayKind | None = Query(default=None),
    quality: PlayQuality | None = Query(default=None),
) -> PlayQuery:
    ...
    if host is not None and host not in _plex_host_names():
...
def plays_overview(query: PlayQuery = Depends(play_query)) -> PlaysOverviewView:
    ...
    """
    with db.session(config.db_path()) as connection:
        data = plays.overview(connection, query.filters, hosts=_plex_host_names(), now=query.now)
    return PlaysOverviewView(
        window=query.window,
        totals=data.totals,
        by_kind=list(data.by_kind),
        by_quality=list(data.by_quality),
        by_host=list(data.by_host),
        timeline=data.timeline,
        top_viewers=list(data.top_viewers),
        top_titles=list(data.top_titles),
    )
...
def plays_users(query: PlayQuery = Depends(play_query)) -> PlayUsersView:
    ...
    with db.session(config.db_path()) as connection:
        return PlayUsersView(users=list(plays.users(connection, query.filters)))
...
def plays_top(
    query: PlayQuery = Depends(play_query),
    metric: TopMetric = Query(default="plays"),
    ...
    return TopTitlesView(metric=metric, titles=list(titles))
...
def plays_sync() -> PlaySyncView:
    ...
    fleet."""
    hosts = tuple((host.name, host.plex_url) for host in config.HOSTS if host.plex_url)
    with db.session(config.db_path()) as connection:
        servers = plays.sync_status(connection, hosts=hosts)
    return PlaySyncView(lookback_days=config.plex_lookback_days(), servers=list(servers))
